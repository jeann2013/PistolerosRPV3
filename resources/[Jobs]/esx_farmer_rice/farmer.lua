...
function Spawn(Pcoords, Pped)	
	ESX.Game.SpawnVehicle(1641462412, Pcoords, 13, function(spawnedVehicle)
		-- save & set plate
		local plate = 'TRABAJO' .. math.random(100, 900)
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, config.zones.process.coords, true) < 1 then
			if not isProcessing then
				ESX.ShowHelpNotification("Presiona ~INPUT_CONTEXT~ para ~g~Procesar")
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if nearbyObject then
					if not isPickingUp and GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "TRACTOR" then
						ESX.ShowHelpNotification("Presione ~INPUT_THROW_GRENADE~ para ~g~Cosechar")
					end
 ...
 TriggerEvent("mythic_progbar:client:progress", {
							...
							controlDisables = {
								...
								ESX.ShowNotification("Necesitas comprar Hoz")
							end
	...
	while true do
		...
		if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			if GetDistanceBetweenCoords(coords, config.zones.cow.coords, true) < 2 then
				...
				ESX.ShowHelpNotification("Presione ~INPUT_CONTEXT~ para ~g~Sacar un trator")
				if not Tractor then	
