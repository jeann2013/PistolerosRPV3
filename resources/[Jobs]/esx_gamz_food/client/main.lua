...
FoodStand = function(place)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'food_stand',
    {
        ...
        align    = 'center',
        elements = {
            { ["label"] = "Comida", ["type"] = "eatable" },
            { ["label"] = "Bebida", ["type"] = "drink" }
        }
    }, function(data, menu)
...
FoodMenu = function(place, type)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'food_menu',
    {
        ...
        if MoneyCheck(data.current.price * 10) then
            TriggerServerEvent("gamz-food:removeMoney", data.current.price * 10)
            ESX.ShowNotification("Comprastes una " .. data.current.label)            
            TriggerServerEvent("gamz-food:addItem", data.current.label2)
        else
            ESX.ShowNotification("No tienes suficientes efectivo")
        end
