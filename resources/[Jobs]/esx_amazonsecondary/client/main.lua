...
function IsJobTrucker()
	if PlayerData ~= nil then
		...
		if PlayerData.job.name ~= nil and PlayerData.job.name == 'amazon' then
			isJobTrucker = true
		end
		return isJobTrucker
	end
...
AddEventHandler('esx_amazonjobsecondary:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_amazonjobsecondary:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()    
    CurrentAction = nil
    CurrentActionMsg = ''
end)
...
function donnerlapaye()
	...
	if vievehicule >= 1 then
		if livraisonTotalPaye == 0 then
			...
			TriggerServerEvent("esx_amazonjobsecondary:pay", amount)
	...
	while true do
		...
		if IsJobTrucker() then
			...
			if isInMarker and not hasAlreadyEnteredMarker then
				...
				TriggerEvent('esx_amazonjobsecondary:hasEnteredMarker', currentZone)
			end

			if not isInMarker and hasAlreadyEnteredMarker then
				hasAlreadyEnteredMarker = false
				TriggerEvent('esx_amazonjobsecondary:hasExitedMarker', lastZone)
			end
