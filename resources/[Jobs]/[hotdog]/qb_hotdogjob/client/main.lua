                ...
                if distance < 10 then
                    ...
                    if not IsWorking then
                        if distance < OffsetData.Distance then
                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Begin working")
                            if IsControlJustPressed(0, Keys["E"]) then
                                StartWorking()
                            end
                        elseif distance < 3 then
                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "Begin working")
                        end
                    else
                        if distance < OffsetData.Distance then
...
function StartWorking()
    ESX.TriggerServerCallback('qb-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            ...
            UpdateUI()
            CheckLoop()
            exports['mythic_notify']:DoHudText('inform', 'You have paid a 1000')
        else
            exports['mythic_notify']:DoHudText('inform', 'You have no money for the deposit..')
        end
    end)
end
                ...
                if ClosestObject ~= nil and ClosestObject == StandObject then
                    ...
                    if ObjectDistance < 1.0 then
                        if not IsPushing then
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Pick stall')
                            if IsControlJustPressed(0, Keys["E"]) then
                                TakeHotdogStand()
                            end
                        else
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Let go of the stall')
                            if IsControlJustPressed(0, Keys["E"]) then
                                LetKraamLose()
                            end
                        end
                    elseif ObjectDistance < 3.0 then
                        DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, 'Picking stall')
                    end
                end
            else
                break
...
function ToggleSell()
    ...
    if StandObject ~= nil then
        if dist < 5.0 then
            ...
            exports['mythic_notify']:DoHudText('inform', 'You ar to far from your hotdog stall..')
        end
    else
        exports['mythic_notify']:DoHudText('inform', 'You have no hotdog stall')
    end
end
...
function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
        ...
        if PlayerDist > 15.0 then
            ...
            exports['mythic_notify']:DoHudText('inform', 'You\'re to far from the stall..')
            break
        end
        Citizen.Wait(100)
    end
    ...
    while pedDist < OffsetData.Distance and SellingData.HasTarget do
        ...
        if PlayerDist < 7.5 then
            if SellingData.Hotdog ~= nil then
                ...
                DrawText3Ds(pedCoords.x, pedCoords.y, pedCoords.z, '[7] Sale '..HotdogsForSale..'x voor €'..(HotdogsForSale * SellingPrice)..',- / [8] Afwijzen')
                if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                    exports['mythic_notify']:DoHudText('type', HotdogsForSale..'x Hotdog(\'s) verkocht voor €'..(HotdogsForSale * SellingPrice)..',-')
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    exports['mythic_notify']:DoHudText('inform', 'Custommer refused!')
                    SellingData.HasTarget = false
                ...
                SellingData.Hotdog = GetAvailableHotdog()
                DrawText3Ds(pedCoords.x, pedCoords.y, pedCoords.z, 'You have no hotdogs.. / [8] Refuse cutommer')
            ...
            exports['mythic_notify']:DoHudText('inform', 'You\'re to far from the stal..')
            break
        end
...
function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        ...
        if Config.MyLevel == 1 then
            exports['mythic_notify']:DoHudText('inform', 'You made a  '..Config.Stock[Quality].Label..' Hotdog !')
            ...
            if Luck == LuckyNumber then
                exports['mythic_notify']:DoHudText('inform', 'You have '..LuckyAmount..' '..Config.Stock[Quality].Label..' Hotdog\'s !')
        ...
        exports['mythic_notify']:DoHudText('inform', 'You have  ('..Config.Stock[Quality].Label..') no hotdogs..')
    end
    PreparingFood = false
end
    ...
    if DoesEntityExist(StandObject) then
        ESX.TriggerServerCallback('qb-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                ...
                exports['mythic_notify']:DoHudText('inform', 'You recieved your 1000')
            else
                exports['mythic_notify']:DoHudText('inform', 'Something is wrong meine freunden..')
            end
        end)
    else
        exports['mythic_notify']:DoHudText('inform', 'Your stall is no where to see you wont recieve you\'re deposit')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsSelling = false
