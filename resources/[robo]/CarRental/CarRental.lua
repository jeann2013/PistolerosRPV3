...
Citizen.CreateThread(function()
	...
	WarMenu.CreateMenu('carRental', 'Transportes John')
	WarMenu.CreateSubMenu('closeMenu', 'carRental', '¿Estás seguro?')
	WarMenu.CreateSubMenu('carPicker', 'carRental', '')
	WarMenu.CreateSubMenu('carInsurance', 'carRental', 'Want to buy car insurance?')
	WarMenu.CreateMenu('carReturn', 'Transportes John')
	WarMenu.SetSubTitle('carReturn', 'Contacto John.')
	...
	while true do
		...
		if WarMenu.IsMenuOpened('carRental') then
			if WarMenu.MenuButton('Tocar la puerta', 'carPicker') then
			...
			WarMenu.SetSubTitle('carRental', 'Puerta principal')
		...
		elseif WarMenu.IsMenuOpened('carPicker') then
			if WarMenu.Button('Pedir Furgoneta') then
				SpawnVehicle("Boxville2")
				TriggerServerEvent("chargePlayer", 500)
				ESX.ShowNotification("Ten las llaves, serán $500 por hoy, lo quiero a primera hora!")
				autoChargeAmount = 0
				isBeingCharged = true
				WarMenu.CloseMenu()
		...
		elseif WarMenu.IsMenuOpened('carReturn') then
			if WarMenu.Button('Devolver la furgoneta') then
				returnVehicle()
				WarMenu.CloseMenu() 
		...
		elseif WarMenu.IsMenuOpened('arrestCheck') then
			if WarMenu.Button('Yes') then
				...
				ESX.ShowNotification('Se ha cancelado el seguro del vehiculo')
				WarMenu.CloseMenu()
			elseif WarMenu.Button('No') then
				WarMenu.CloseMenu()
				arrestCheckAlreadyRan = true
...
Citizen.CreateThread(function()
	while true do
		...
		if isBeingCharged == true then
			...
			ESX.ShowNotification("Cuidado con la policia, quiero mi furgoneta de vuelta!")
		end
	end
end)
...
function returnVehicle()
			...
			damageInsurance = false
			damageCharge = false
			ESX.ShowNotification("Al pasar el tiempo un trabajador de John habría pasado a buscar el vehículo.")
			local currentVehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
			SetEntityAsMissionEntity(currentVehicle, true, true)
			local x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(-1), false))
