		...
		if isHolding then
			...
			if IsControlJustPressed(0, 51) then
				...
				if isInHouse then
					...
					if Vdist(markerCoords, coords) < 3 then
						ShowSubtitle("Objeto robado, vuelve a entrar para robar mas!.")
			...
			for _,pickup in pairs(pickups) do
				...
				if onMission and Vdist(coords, GetEntityCoords(pickup.prop)) < 2 and not isHolding then
					DisplayHelpText("Presiona ~INPUT_CONTEXT~ para robar esto")
					...
					if IsControlJustPressed(0, 51) then
						...
						ShowSubtitle("Tomas un objeto ve a dejarlo a la van!")
