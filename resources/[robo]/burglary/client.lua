		...
		if IsControlJustPressed(0, 51) and not onMission and IsPedInAnyVehicle(PlayerPedId()) then
			...
			if IsMissionVehicle(GetEntityModel(veh)) then
				...
				if time >= 0 and time <= TimeToSeconds(5, 30, 0) then
					...
					ShowMPMessage("Ratero", "encuentra una ~r~casa ~s~para robar.", 3500)
					--ShowSubtitle("encuentra una ~r~casa ~s~ para robar")
				else
					DisplayHelpText("")
				end
			end
...
CreateThread(function()
	while true do
		...
		if onMission then
			...
			DrawTimerBar("LUZ DE DIA", AddLeadingZero(time.hours) .. ":" .. AddLeadingZero(time.minutes), 1)
		end
	end
end)
		...
		if onMission then
			for k,door in pairs(doors) do
				...
				if Vdist(coords, door.coords.x, door.coords.y, door.coords.z) < 2 then
					DisplayHelpText("Presiona ~INPUT_CONTEXT~ para entrar a la casa.")
					...
					if IsControlJustPressed(0, 51) then
						...
						ShowSubtitle("Esta dentro de la casa debes ser cuidadoso!")
					end
				end
			end
		end
