	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Navegar con ~y~W,A,S,D~s~ y confirmar con ~y~SPACE~s~ para el código de la izquierda.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Utilizar ~y~Teclas de flecha~s~ y ~y~ENTER~s~ para el código de la derecha")
			else
