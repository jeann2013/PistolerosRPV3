    ...
    Citizen.CreateThread(function()
        ...
        if not currentrobbing then 
            if Config.MustUseVoice then
                ESX.Game.Utils.DrawText3D(roblocalcoords, "Asaltar...", 0.25)
            else
                ESX.Game.Utils.DrawText3D(roblocalcoords, "[~g~E~s~] para asaltar", 0.25)
            end
        elseif lasttargetPed == targetPed then
            ESX.Game.Utils.DrawText3D(roblocalcoords, "Ya fue asaltado..", 0.25)
        else
            if not Config.progressBars then
            ESX.Game.Utils.DrawText3D(roblocalcoords, "Asaltando..", 0.25)
            end
        end
        ...
        if notneeded then
            ...
            if not currentrobbing then
                ...
                if Config.progressBars then
                    exports['progressBars']:startUI(Config.RobWaitTime * 1000, "Asaltando...")
                end 
                ...
                if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, tCoords.x, tCoords.y, tCoords.z, true) <= 5.0 then 
                    if not IsPedDeadOrDying(targetPed) then
                        ...
                        additems = math.random(1,3)
                        print('additems',additems)
                        print('Config.AddItemsPerctent',Config.AddItemsPerctent)
