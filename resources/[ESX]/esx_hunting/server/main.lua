...
AddEventHandler('esx-qalle-hunting:sell', function()
    ...
    local MeatPrice = 1250
    local LeatherPrice = 250
    ...
    if MeatQuantity > 0 or LeatherQuantity > 0 then
        ...
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'Vendistes ' .. LeatherQuantity + MeatQuantity .. ' y ganastes $' .. LeatherPrice * LeatherQuantity + MeatPrice * MeatQuantity)
    else
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'No tienes mas carne ni piel')
    end
