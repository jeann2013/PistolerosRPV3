...
local Positions = {
	['StartHunting'] = { ['hint'] = '[E] Inicia la caza', ['x'] = -769.23773193359, ['y'] = 5595.6215820313, ['z'] = 33.48571395874 },
	['Sell'] = { ['hint'] = '[E] Venta', ['x'] = 969.16375732422, ['y'] = -2107.9033203125, ['z'] = 31.475671768188 },
	...
	Citizen.CreateThread(function()
		for index, v in ipairs(Positions) do			
			if index ~= 'SpawnATV' then
				...
				AddTextComponentString('Punto de entrada')
...
function StartHuntingSession()
	...
	if OnGoingHuntSession then
		...
		HuntCar = CreateVehicle(GetHashKey('enduro'), Positions['SpawnATV'].x, Positions['SpawnATV'].y, Positions['SpawnATV'].z, 169.79, true, false)
		...
		Citizen.CreateThread(function()
			...
			for index, value in pairs(AnimalPositions) do
				...
				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString('Ciervos - Animales para Cazar')
					...
					if DoesEntityExist(value.id) then
						...
						if AnimalHealth <= 0 then
							...
							if PlyToAnimal < 2.0 then
								...
								ESX.Game.Utils.DrawText3D({x = AnimalCoords.x, y = AnimalCoords.y, z = AnimalCoords.z + 1}, '[E] Descuartiza al animal', 0.4)
...
function SlaughterAnimal(AnimalId)
	...
	ESX.ShowNotification('Mataste al animal y recibiste carne de  ' ..AnimalWeight.. 'kg')
