		...
		if IsPedSittingInAnyVehicle(PlayerPedId()) then 
			for i = 1, #vehicleWashStation do
				...
				if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), garageCoords2[1], garageCoords2[2], garageCoords2[3], true ) < 5 then
					if mycie == false then
					HelpPromt('Presione ~INPUT_CONTEXT~ para ~b~lavar ~s~vehículo for ~g~(25$)')
					if IsControlJustPressed(1, Key) then
						TriggerServerEvent('es_better_carwash:checkmoney')
					end
				else
					HelpPromt('Presione ~INPUT_CONTEXT~ para ~o~detener')
					if IsControlJustPressed(1, Key) then
						mycie = false
						timer2 = false
    ...
    Citizen.CreateThread(function()
		while timer2 do
			...
			elseif (timer == 0) then
				...
				TriggerEvent("pNotify:SendNotification", {text = "Tu vehículo ha sido lavado! por 25$", type = "warning", queue = "global", timeout = 5000, layout = "centerLeft"})
				StopParticleFxLooped(particles, 0)
				StopParticleFxLooped(particles2, 0)
				timer2 = false
			end
        ...
        while true do
			...
			if mycie == true then
				...
				if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), garageCoords3[1], garageCoords3[2], garageCoords3[3], true ) < 4 then
					DrawText3D(garageCoords3[1], garageCoords3[2], garageCoords3[3] + 3, '~b~Lavando... ~s~Tiempo:~b~ '.. timer ..' ~s~segundos.')
				end
				end
			end
		end
...
AddEventHandler('es_better_carwash:notenoughmoney', function (moneyleft)
	TriggerEvent("pNotify:SendNotification", {text = "No tienes sufuciente dinero! Necesitas:" .. moneyleft .."$ mas.", type = "advertencia", queue = "global", timeout = 5000, layout = "centerLeft"}) 
end)
