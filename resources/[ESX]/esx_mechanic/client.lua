...
function GoToTarget(x, y, z, vehicle, driver, vehhash, target)
    ...
    ShowAdvancedNotification(companyIcon, companyName, "Mecánico enviada", "Se ha enviado un mecánico a su ubicación. Gracias por usar ~y~" .. companyName)
    enroute = true
    while enroute do
        Citizen.Wait(500)
...
function RepairVehicle(target, vehicle, driver)
    ...
    Citizen.Wait(500)
	ShowAdvancedNotification(mechPedPick.icon, mechPedPick.name, "Vehículo reparado" , mechPedPick.lines[math.random(#mechPedPick.lines)])
...
function GetTargetVehicle(player, dir)
    ...
    if DoesEntityExist(dmgVeh) then
        ...
        ShowNotification("No se pudo encontrar un vehículo.")
    end
end
