...
AddEventHandler("stadus_skills:addStamina", function(source, amount)
	...
	TriggerClientEvent('esx:showNotification', source, 'Te sientes ~g~' .. round(amount, 2) .. '% ~s~mas rapido!')
	MySQL.Async.fetchAll('SELECT * FROM `stadus_skills` WHERE `identifier` = @identifier', {['@identifier'] = xPlayer.identifier}, function(skillInfo)
...
AddEventHandler("stadus_skills:addStrength", function(source, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	TriggerClientEvent('esx:showNotification', source, 'Te sientes mas ~g~' .. round(amount, 2) .. '% ~s~fuerte!')
	MySQL.Async.fetchAll('SELECT * FROM `stadus_skills` WHERE `identifier` = @identifier', {['@identifier'] = xPlayer.identifier}, function(skillInfo)
...
AddEventHandler("stadus_skills:addDriving", function(source, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	TriggerClientEvent('esx:showNotification', source, 'Eres ~g~' .. round(amount, 2) .. '% ~s~mejor conduciendo!')
	MySQL.Async.fetchAll('SELECT * FROM `stadus_skills` WHERE `identifier` = @identifier', {['@identifier'] = xPlayer.identifier}, function(skillInfo)
...
AddEventHandler("stadus_skills:addFishing", function(source, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	TriggerClientEvent('esx:showNotification', source, 'Eres ~g~' .. round(amount, 2) .. '% ~s~ mejor pescando!')
	MySQL.Async.fetchAll('SELECT * FROM `stadus_skills` WHERE `identifier` = @identifier', {['@identifier'] = xPlayer.identifier}, function(skillInfo)
...
AddEventHandler("stadus_skills:addDrugs", function(source, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	TriggerClientEvent('esx:showNotification', source, 'Eres ~g~' .. round(amount, 2) .. '% ~s~mejor tomando ~y~drogas~s~!')
	MySQL.Async.fetchAll('SELECT * FROM `stadus_skills` WHERE `identifier` = @identifier', {['@identifier'] = xPlayer.identifier}, function(skillInfo)
