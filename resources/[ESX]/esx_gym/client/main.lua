...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presione ~INPUT_CONTEXT~ para alquilar una ~b~bici')
				
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						ESX.ShowNotification("Tienes una ~r~bici")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presione ~INPUT_CONTEXT~ para abrir el menu ~b~gym~w~')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presione ~INPUT_CONTEXT~ para hacer ejercicios con tus ~g~brasos')
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then
							...
							ESX.ShowNotification("Necesitas descansar ~r~60 segundos ~w~antes de hacer otro ejercicio.")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Necesita una membresía para hacer ~r~ejercicios")
						end
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presione ~INPUT_CONTEXT~ para hacer unas ~g~dominadas')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("Preparando para hacer algun ~g~ejercicio~w~...")
						Citizen.Wait(1000)					
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presione ~INPUT_CONTEXT~ para hacer unas ~g~largartijas')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("Preparando para hacer ~g~ejercicio~w~...")
						Citizen.Wait(1000)					
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(situps) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presione ~INPUT_CONTEXT~ para hacer unas ~g~largatijas')
...
function CheckTraining()
	...
	if resting == false then
		ESX.ShowNotification("Ahora puedes volver a hacer ejercicio...")
	end
end
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Estamos abiertos ~g~24~w~ horas/ dia. Bienvenido!")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            title    = 'Gym - Shop',
            elements = {
				{label = 'Batido de proteina ($6)', value = 'protein_shake'},
				{label = 'Agua ($1)', value = 'water'},
				{label = 'AlmuerzoDeportivo ($2)', value = 'sportlunch'},
				{label = 'Powerade ($4)', value = 'powerade'},
				{label = 'Benda ($50)', value = 'bandage'},
            }
        },
        function(data, menu)
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_ship_menu',
        {
            title    = 'Gym - Membresia',
            elements = {
				{label = 'Membresia ($800)', value = 'membership'},
            }
        },
        function(data, menu)
