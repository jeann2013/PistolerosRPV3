...
function OpenMenuGarage(PointType)
	...
	if PointType == 'spawn' then
		table.insert(elements,{label = "Lista de vehículos", value = 'list_vehicles'})
	end

	if PointType == 'delete' then
		table.insert(elements,{label = "Ingresar vehículos", value = 'stock_vehicle'})
	end

	if PointType == 'pound' then
		table.insert(elements,{label = "Devolución del vehículo ("..Config.Price.."$)", value = 'return_vehicle'})
	end
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			SpawnVehicleEdenGarage(data.current.value)
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		...
		for k,v in ipairs(vehicles) do
			...
			if v.state then
				...
				vehicleLabel = vehicleName..': Depósito'
			end
		...
		function(data, menu)
			if data.current.state then
				...
				SpawnVehicleEdenGarage(vehicleProps)
			else
				TriggerEvent('esx:showNotification', 'Su vehículo está en el depósito')
			end
		end,
		function(data, menu)
			menu.close()
...
function reparation(prix,vehicle,vehicleProps)
	...
	local elements = {
		{label = "Ingrese vehículo por ("..prix.."$), para repararlo ", value = 'yes'},
		{label = "ve a ver al mecánico", value = 'no'},
	}
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if(data.current.value == 'no') then
				ESX.ShowNotification('Ven a ver al mecánico')
			end
...
function StockVehicleMenu()
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
			...
			if (valid) then
				TriggerServerEvent('eden_garage:debug', "placa del vehículo devuelta al garaje: "  .. vehicleProps.plate)
				TriggerServerEvent('eden_garage:logging',"la salud del vehículo regresó al garaje: " .. engineHealth)
		...
		TriggerEvent('esx:showNotification', 'No hay vehículo para entrar')
	end
...
function SpawnVehicleEdenGarage(vehicle)
...
AddEventHandler('eden_garage:hasEnteredMarker', function(zone)
	...
	if zone == 'spawn' then
		CurrentAction     = 'spawn'
		CurrentActionMsg  = "Presiona ~INPUT_PICKUP~ para sacar vehículo"
		CurrentActionData = {}
	end

	if zone == 'delete' then
		CurrentAction     = 'delete'
		CurrentActionMsg  = "Presiona ~INPUT_PICKUP~ para ingresar vehículo"
		CurrentActionData = {}
	end

	if zone == 'pound' then
		CurrentAction     = 'pound_action_menu'
		CurrentActionMsg  = "Presiona ~INPUT_PICKUP~para acceder al deposito"
		CurrentActionData = {}
	end
end)
