...
AddEventHandler('eden_garage:modifystate', function(vehicleProps, state)
	...
	for _,v in pairs(vehicules) do
		if(plate == v.plate)then
			local idveh = v.id
			MySQL.Sync.execute("UPDATE owned_vehicles SET state =@state WHERE vehicle=@id",{['@state'] = state , ['@id'] = v.id})
			break
		end
	end
end)
...
AddEventHandler('eden_garage:pay', function()
	...
	TriggerClientEvent('esx:showNotification', source, 'Pagastes ' .. Config.Price)
...
AddEventHandler('eden_garage:payhealth', function(price)
	...
	TriggerClientEvent('esx:showNotification', source, 'Pagastes ' .. price..' $')
