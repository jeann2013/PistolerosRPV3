...
local admin = menu:AddButton({ icon = '👑', label = 'Admin', value = m2, description = 'Spectate / Ban / Warn' })
...
local test = m2:AddCheckbox({ icon = '⚙️', label = 'Open Logs', value = pil, description = 'Open Logs Connections/Kill' })
...
test:On('check', function(i)
TriggerEvent('logs:openmenutahlesouf')
end)
test:On('uncheck', function(i)
  print('SP Admin Menu')
end)
...
modo_showcoord:On('check', function(i)
  Admin.showcoords = not Admin.showcoords  
    ESX.ShowNotification('Show Coords : ~g~ON')  
end)
modo_showcoord:On('uncheck', function(i)
  Admin.showcoords = not Admin.showcoords    
    ESX.ShowNotification('Show Coords : ~r~OFF')
end)
noclip:On('check', function(i)
  admin_no_clip()
  ESX.ShowNotification('No Clip : ~g~ON')
end)
noclip:On('uncheck', function(i)
  admin_no_clip()
  ESX.ShowNotification('No Clip : ~r~OFF')
end)
godmode:On('check', function(i)
 Admin.godmode = not Admin.godmode
    if Admin.godmode then
    SetEntityInvincible(PlayerPedId(), true)
    ESX.ShowNotification('GodMod : ~g~ON')
    else
    SetEntityInvincible(PlayerPedId(), false)
    ESX.ShowNotification('GodMod : ~r~OFF')
	end
end)
